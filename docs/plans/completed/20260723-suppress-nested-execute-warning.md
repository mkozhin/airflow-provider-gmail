...
downstream корректны). Полная диагностика зафиксирована в разделе Context ниже
(исходно она велась в `todo.md`, теперь удалённом).
...
- [x] удалить `todo.md` из корня (диагностика перенесена в этот план/CHANGELOG)
- [x] переместить этот план в `docs/plans/completed/` (moved by harness after all phases)
- [x] обновлять `AGENTS.md`/`CLAUDE.md` **не требуется** — разовая особенность
