...
- [x] Перемещение в `docs/plans/completed/` — **выполнит харнесс на шаге завершения
      exec** (файл пока в активной директории — это ожидаемо). Ветку не мержить,
      пока 2.9.1 CI не зелёная.
- [x] `AGENTS.md`/`CLAUDE.md` — обновление не требуется (правка только в тест-файле,
      не новое общепроектное соглашение)
- [x] `CHANGELOG.md` — запись **не требуется**: правка только в тест-файле
